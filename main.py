...
client = commands.Bot(
    command_prefix=".", intents=intents, help_command=None, allowed_mentions=None
)
...
async def on_message(message):
    ...
    if client.user.mentioned_in(message):
        ...
        embed.add_field(
            name="Vote and support",
            value="please vote for this bot [here](https://top.gg/bot/835373122978578463/vote)\n Give us your reviews [here](https://top.gg/bot/835373122978578463)",
        )
        ...
        await message.channel.send(
            "You have to specify whether you want to `.loopq` or `.loopsong` for more info type '.help'."
        )
...
first.add_field(
    name="Music",
    value="These commands are used for playing music in your voice chat",
    inline=False,
)
first.add_field(
    name="'.play'",
    value="Use this command and the name of the song you want to play to listen to it in your voice chat. Soundcloud, Youtube music, Spotify playlists are supported.",
    inline=False,
)
first.add_field(
    name="'.pause'", value="Use this command to pause the currently playing song."
)
first.add_field(
    name="'.resume'", value="Use this command to resume a paused song.", inline=True
)
first.add_field(
    name="'.skip `<time in seconds>`'",
    value="Use this command and specify the number of seconds you wanna skip, if not specified, the entire song will skip.",
)
first.add_field(
    name="'.queue' or '.q'",
    value="Use this command to have a look at all your queued songs.",
    inline=True,
)
first.add_field(
    name="'.nowplaying' or '.np'",
    value="Use this command to look at the currently playing song.",
)
first.add_field(
    name="'.disconnect' or 'leave'",
    value="Use this command to make the bot leave the voice chat.",
)
...
second.add_field(
    name="'.playnext' or 'pn'",
    value="Use this command to play the song your liking next.",
)
second.add_field(
    name="'.rewind `<time in seconds>`'",
    value="Use this command and specify the number of seconds you wanna rewind, if not specified, the entire song will rewind.",
)
second.add_field(
    name="'.shuffle'", value="Use this command to shuffle the queued songs."
)
second.add_field(
    name="'.lyrics `<name of artist - name of song>`'",
    value="This command to get the lyrics of the currently playing song right to your text channel, if a name is specified, you'll get that songs lyrics.",
)
second.add_field(
    name="'findsong' or '.getsong'",
    value="Ever forget the name of some song but remember the lyrics? We've got you covered!",
)
second.add_field(
    name="'.previous' or '.prev'",
    value="Play the last playing song in your vc. You can only use this command for the last 10 songs.",
)
...
third.add_field(
    name="'.loopqueue' or '.lq'",
    value="Use this command to play the current queue on loop. This is a toggle command, you'll need to call this command again to exit.",
)
third.add_field(
    name="'.looptrack' or '.ls'",
    value="Use this command to play the currently playing song on loop. This is a toggle command, you'll need to call this command again to exit.",
)
third.add_field(
    name="'.volume' or '.vol'",
    value="Use this command to change volume of the player between 1-100.",
)
third.add_field(name="'.clearqueue' or '.clearq'", value="Use this to clear the queue.")
third.add_field(
    name="'.change' or '.put' `<pos of song>` `<pos you want to change to>`",
    value="Use this command to change the pos of a song in the Queue. Type 'last' or 'first' to put the chosen song to fisrt or last.",
)
third.add_field(
    name="'.remove' or '.r' `<position>`",
    value="Use this command to remove the song at the specified Queue position. Use the queue command to get the queue position of each song.",
)
...
async def help(ctx):
    embed = discord.Embed(
        ...
        url="https://top.gg/bot/835373122978578463",
    ...
    embed.add_field(
        name="Moderation", value="For the moderation commands react to the ⚖️"
    )
    embed.add_field(
        name="Vote and support",
        value="please vote for this bot [here](https://top.gg/bot/835373122978578463/vote)\n\n Give us your reviews [here](https://top.gg/bot/835373122978578463)",
    ...
    embed.add_field(
        name="Invite",
        value="Like this bot? Want it in your server?\n"
        "Type **'.invite'**\n This will give you an invite link in the server and you will also get a DM. ;-)",
        inline=False,
    )
    ...
    def check(reaction, user):
        return user == ctx.author and (
            str(reaction.emoji) == "🎵" or str(reaction.emoji) == "⚖️"
        )
    ...
    try:
        ...
        if reaction.emoji == "🎵":
            ...
            embed = discord.Embed(
                title="Moderation still under construction!",
                description="Moderation commands will be out soon!!",
                color=discord.Color.from_rgb(132, 112, 255),
            )
...
async def invite(ctx):
    embed = discord.Embed(
        title="Thank You for wanting to Invite this bot!!",
...
async def byebye(ctx):
    goodbye = discord.Embed(
        ...
        description="Mr. Wonka will officially be shut down today, for various reasons we.. I cannot disclose, so, i guess this is goodbye. The bot will be back though, but not anytime soon...",
        color=discord.Color.from_rgb(132, 112, 255),
    )
    goodbye.set_footer(
        text="Thank you to all those who used this bot\nAnd those who didn't, you missed out (i guess)"
    )
...
async def privacy(ctx):
    ...
    embed.add_field(
        name="Contact",
        value="At the moment we don't have any help server (users will be infored when we make one) so, for the moment please contact the Main dev `Lushain#7914` for any help or query.",
    )
    embed.add_field(
        name="Get data removed",
        value="To get all the cached data on your ID Direct Message `Lushain#7914` stating why you need your data removed and how we can help. **No applications to get the user data removed will be denied.**",
    )
    embed.add_field(
        name="Support",
        value="This is an ever growing bot, we are completly open about the data we collect. \nPlease help further support this bot by inviting it to your own servers.\n`.invite`",
        inline=False,
    )
