...
class EvalCommand(commands.Cog):
    ...
    async def _eval(self, ctx, *, code: str):
        ...
        args = {"discord": discord, "client": self.client, "ctx": ctx}
        ...
        try:
            ...
            embed = discord.Embed(
                ...
                description=f"Response time : {round(self.client.latency*1000)} ms",
            )
            embed.add_field(name="Input", value=f"```py\n{input}```", inline=False)
            embed.add_field(
                name="Output :", value=f"```py\n{self.resolve_variable(response)}```"
            ).set_footer(text=f"DATA TYPE : {type(response).__name__}")
