        ...
        while True:
            ...
            if upcoming:
                try:
                    ...
                    fmt = "\n".join(
                        f"[{_.title}]({_.uri}) | `🧿 live`" for _ in upcoming
                    )
            else:
            ...
            embed.set_footer(
                text=f"• Queue Loop: {tick_or_not(self.loop_queue)} | Track Loop: {tick_or_not(self.loop_track)} | Paused: {tick_or_not(player.is_paused)}"
            )
...
class Music(commands.Cog):
    ...
    async def process_request(
        ...
        if isinstance(tracks, wavelink.TrackPlaylist):
            ...
            if position == "pn":
                track = Track(
                    tracks[0].id, tracks[0].info, requester=ctx.author, pn=True
                )
                ...
                track = Track(
                    tracks[0].id, tracks[0].info, requester=ctx.author, pn=False
                )
