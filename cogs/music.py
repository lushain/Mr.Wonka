...
class MusicController:
    def __init__(self, ctx):
        ...
        self.genius = Genius(
            "Zb2AEFCJL7r-nfdjbFeNViR9RoK82J0GxSsk53yQygB7v0u8Elt6UobXi_DUZXmJ"
        )
    ...
    async def controller_loop(self):
        ...
        if song.id == "spotify":
            spotify_tracks = await self.wavelink.get_tracks(
                f"ytsearch:{song.title} {song.author} audio"
            )
        ...
        if upcoming:
            try:
                fmt = "\n".join(
                    f"[{_.title}]({_.uri}) | `{dt.timedelta(milliseconds = _.length)}`"
                    for _ in upcoming
                )
            except OverflowError:
        ...
        try:
            embed = discord.Embed(
                title=f"{self.player.current.title}",
                url=f"{self.player.current.uri}",
        ...
        embed.set_footer(
            text=f"• Queue Loop: {tick_or_not(self.loop_queue)} | Track Loop: {tick_or_not(self.loop_track)} | Paused: {tick_or_not(self.player.is_paused)}"
        )
    ...
    async def check_disconnect(
        ...
        if member == self.ctx.me:
            if before.channel is not None and after.channel is None:
                return self.bot.loop.create_task(self.ctx.cog.cleanup(self.ctx))
...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        self.spotify_client = spotify.Client(
            "c0e3f0b9935a49daa7433052e5fecfb6", "bc9e76bba91d42e9ab924ab5124698cb"
        )
        self.spotify_http_client = spotify.HTTPClient(
            "c0e3f0b9935a49daa7433052e5fecfb6", "bc9e76bba91d42e9ab924ab5124698cb"
        )
        self.channel = None
    ...
    async def process_request(
        self, position: str, ctx: commands.Context, source: str, query
    ):
        ...
        if matches:
                ...
                if search_type == "playlist":
                    try:
                        results = spotify.Playlist(
                            ...
                            data=await self.spotify_http_client.get_playlist(
                                spotify_id
                            ),
                        )
                        ...
                        return await ctx.send(
                            "I was not able to find this playlist! Please try again or use a different link."
                        )

                elif search_type == "album":
                    try:
                        results = await self.spotify_client.get_album(
                            spotify_id=spotify_id
                        )
                        search_tracks = await results.get_all_tracks()
                    except:
                        return await ctx.send(
                            "I was not able to find this album! Please try again or use a different link."
                        )

                elif search_type == "track":
                    try:
                        results = await self.spotify_client.get_track(
                            spotify_id=spotify_id
                        )
                        search_tracks = [results]
                    except:
                        return await ctx.send(
                            "I was not able to find this song! Please try again or use a different link."
                        )
                    tracks = await self.bot.wavelink.get_tracks(
                        f"ytsearch:{search_tracks[0].name} - {search_tracks[0].artists[0].name}"
                    )
                    print(search_tracks[0].name)
                    if not tracks:
                        return await ctx.send(
                            "⚠️ No tracks could be found by the provided link. Make sure, no extra characters were provided in the link, and that the link is to a song or a playlist. "
                        )
            ...
            if not tracks:
                ...
                if not tracks:
                    return await ctx.send(
                        "⚠️ I could not find any results with the provided search query on Youtube or on Soundcloud."
                    )
    ...
    async def on_command_error(self, ctx, error):
        if isinstance(error, commands.MissingRequiredArgument):
            return await ctx.send(
                "⚠️ You have to specify something to use this command. For help write '.help'"
            )
    ...
    async def connect_(self, ctx):
        ...
        if not hasattr(ctx.me.guild_permissions, "speak"):
            embed = discord.Embed(
                color=0x2F3136,
                title="I do not have permissions to speak in the Voice channel!!!",
            )
            return await ctx.send(embed=embed)
            ...
            embed = discord.Embed(
                color=0x2F3136,
                title="I do not have permissions to connect to the voice channel!!",
            )
            return await ctx.send(embed=embed)
    ...
    async def play(self, ctx, *, query: str):
        try:
            ...
            return await ctx.send(
                "You can only use this command when you are connected to a voice channel!"
            )
    ...
    async def play_next(self, ctx, *, query: str):
        ...
        await self.process_request(
            position="pn", ctx=ctx, source="ytsearch", query=query
        )
    ...
    async def now_playing(self, ctx):
        ...
        embed.set_footer(
            text=f"• Queue Loop: {tick_or_not(controller.loop_queue)} | Track Loop: {tick_or_not(controller.loop_track)} | Paused: {tick_or_not(player.is_paused)}\n Type .np for an updated now playing message."
        )
    ...
    async def queue(self, ctx):
        ...
        pages = menus.MenuPages(
            source=MySource(list(controller.queue._queue)), clear_reactions_after=True
        )
        # await ctx.invoke(self.now_playing)
    ...
    async def lyrics(self, ctx, *, given=None):
        ...
        if isinstance(given, str):
            ...
            if player.is_playing:
                ...
                return await ctx.send(
                    "⚠️ No song is being played in the vc and name for no song was given! ⚠️"
                )
        ...
        try:
            ...
            track_name = x[0]
            song_in = controller.genius.search_song(
                title=track_name, get_full_info=False
            )
            NoneType = type(None)
            ...
            try:
                ...
                song_in = controller.genius.search_song(
                    title=track_name, artist=artist_name, get_full_info=False
                )
                NoneType = type(None)
    ...
    async def search_lyrics(self, ctx, *, query: str):
        ...
        embed.set_footer(
            text="Not the songs you were looking for? Try searching with some other set of lyrics."
        )
        await ctx.send(embed=embed)
        ...
        if fmt:
            ...
            check_msg = await ctx.send(
                "Would you like me to play the first result? `(yes/no)`"
            )

            def check(m):
                return (
                    (m.content == "yes" or m.content == "no")
                    ...
                    if not player.is_connected:
                        ...
                        if not player.is_connected:
                            ...
                            await self.process_request(
                                ...
                                ctx=ctx,
                                source="ytsearch",
                                query=f"{val_list[0]} by {key_list[0]}",
                            )
                elif m.content == "no":
    ...
    async def loop_queue(self, ctx):
        ...
        if controller.loop_queue:
            ...
            if controller.loop_track:
                check_msg = await ctx.send(
                    "The Track is already looping, would you like to stop the Track loop and loop this Queue?`(yes/no)`"
                )
                try:
                    m = await self.bot.wait_for("message", timeout=60.0, check=check)
                except asyncio.TimeoutError:
    ...
    async def loop_track(self, ctx):
        ...
        if controller.loop_track:
            ...
            if controller.loop_queue:
                check_msg = await ctx.send(
                    "The Queue is already looping, would you like to stop the Queue loop and loop this song?`(yes/no)`"
                )
                try:
                    m = await self.bot.wait_for("message", timeout=60.0, check=check)
                except asyncio.TimeoutError:
    ...
    async def put(self, ctx, get_pos: int, put_pos):
        ...
        try:
            ...
            return await ctx.send(
                f"The Queue doesn't even have {get_pos} songs in it!!"
            )
        else:
            ...
            if put_pos == "last":
                ...
                try:
                    ...
                    return await ctx.send(
                        "You have to specify the queue position of the song ex `2` or to specify the first and last songs you can say `first` `last`\n ```\n.put 4 last\n```"
                    )
                else:
                    return await ctx.message.add_reaction("👍")
    ...
    async def put_error(self, ctx, error):
        if isinstance(error, commands.BadArgument) or isinstance(
            ...
            return await ctx.send(
                "⚠️ You can use this command to change the position of a song in the queue.\n `Ex: .put <position of the song in queue> <position you want to change it to>`"
            )
        else:
            raise error
    ...
    async def remove(self, ctx, pos="next"):
        ...
        if pos == "next" or pos == "first":
            ...
            try:
                ...
                return await ctx.send(
                    "You have to specify the queue position of the song ex `2` or to specify the next and last song you can say `next` `last`\n ```\n.remove next\n```"
                )
            except IndexError:
                return await ctx.send(
                    "There aren't even that many songs in the queue. Stop trynna break me!!\n`.q`"
                )
